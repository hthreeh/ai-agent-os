...
- search_files: 搜索文件
- create_user: 创建用户（包含"检查用户是否存在"等前置检查，请用pre_check字段，不要单独创建诊断任务）
- delete_user: 删除用户（包含前置检查）
...
- diagnostic: 诊断问题（仅用于"排查XX问题"、"诊断XX故障"等明确的故障诊断请求，不要用于"检查用户是否存在"等前置检查）

重要规则：
1. "检查用户是否存在"不是diagnostic，而是create_user的前置检查，用pre_check字段实现
2. "检查端口是否开放"不是diagnostic，而是port_status或manage_service的前置检查
3. diagnostic只用于明确的故障诊断场景，如"80端口无法访问"、"服务启动失败"等
